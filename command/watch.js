...
module.exports = {
  run: async option => {
    logger.log(chalk.cyanBright(`Start running Wits watch mode............\n`));

    await util.initTools();

    const data = await userInfoHelper.getLatestWitsconfigInfo().connectionInfo;
    ...
    try {
      data.isDebugMode
        ? appLaunchHelper.launchDebugMode(deviceName, hostAppId, data.deviceIp)
