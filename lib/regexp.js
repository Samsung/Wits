...
module.exports = {
    REMOTE_URI: new RegExp(/(http|ftp|https):\/\/([\w+?\.\w+])+([a-zA-Z0-9\~\!\@\#\$\%\^\&\*\(\)_\-\=\+\\\/\?\.\:\,\,]*)?/),
    COMMENT: new RegExp(/<!--\s*.*?\s*-->/g),
    HOST_DATA: new RegExp(/({{CONTENT_PATH}})|({{HOST_IP}})|({{HOST_PORT}})|({{CONTENT_SRC}})|({{HOST_BASE_CONTENT_PATH}})/g),
