...
function decryptPassword(pwdFile) {
    ...
    if (util.PLATFORM === 'win32') {
        try {
            const out = execSync(`${util.TOOLS_CRYPT_PATH} --decrypt ${pwdFile}`);
        ...
        const out = execSync(`${util.TOOLS_CRYPT_PATH} lookup --label="tizen-studio" keyfile ${pwdFile} tool certificate-manager`).toString();
        if (out) {
        ...
        const out = execSync(`security find-generic-password -wa ${pwdFile} -s certificate-manager`).toString();
        if (out) {
            console.log(`out: ${out}, length: ${out.length}`);
            out.trim();
            password = out.replace(/[\r\n]/g,"");
