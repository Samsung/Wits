...
var AUTOR_SIGNATURE = path.join('.sign', 'author-signature.xml');
var PUBLIC_SIGNATURE = path.join('.sign', 'signature1.xml');
var PUBLIC_SIGNATURE2 = path.join('.sign', 'signature2.xml');
...
function createSignatureXML(signatureId, destfile) {
    console.log('@@@@@@@@@@@@@@@@ createSignatureXML');
    //get signInfo xml
...
function signPackage(expli) {
    console.log('==============================Signature Package start!');

    let workspacePath = path.join(util.WITS_BASE_PATH, '../', 'container');
    if (typeof workspacePath == 'undefined') {
        var noWorkspace =
            "There's no web app project in workspace, please check!";
        console.error(noWorkspace);
