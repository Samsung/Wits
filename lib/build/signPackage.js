...
const userInfoHelper = require('../userInfoHelper');

// 3rd packager tool, enter into  'xml-crypto' to support xml signature
var Dom = require('xmldom').DOMParser;
var ExclusiveCanonicalization = require('./exclusive-canonicalization')
...
const signPath = path.join(util.WITS_BASE_PATH, '../', 'container', '.sign')
var AUTOR_SIGNATURE = path.join(util.WITS_BASE_PATH, '../', 'container', '.sign', 'author-signature.xml');
var PUBLIC_SIGNATURE = path.join(util.WITS_BASE_PATH, '../', 'container', '.sign', 'signature1.xml');
var PUBLIC_SIGNATURE2 = path.join(util.WITS_BASE_PATH, '../', 'container', '.sign', 'signature2.xml');

// Module name
var moduleName = 'signPackage';
//get the filename(contain relative filepath) in test app in workspace
var workspacePath = path.join(util.WITS_BASE_PATH, '../', 'container');
var digestURI = [];
...
function createSignatureXML(signatureId, destfile) {
    console.log('createSignatureXML: ', destfile);
    //get signInfo xml
...
function signPackage(profilePath) {
    ...
    console.log('signPackage1');
    // Sign package
    p12ToPem.createDir(signPath);
