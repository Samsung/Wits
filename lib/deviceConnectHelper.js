...
module.exports = {
  getConnectedDeviceInfo: async (deviceIpAddress, port) => {
    module.exports.connectDevice(deviceIpAddress, port);
  ...
  connectDevice: (deviceIpAddress, port) => {
    if (!util.isIpAddress(deviceIpAddress)) {
      throw logger.error(
        chalk.red('Invalid format of device IP address, please check it again.')
      );
    }
    let connectPort = port ? port : TV_CONNECT_PORT;
    if (deviceIpAddress !== EMULATOR_IP) {
      logger.log(`connect to....${deviceIpAddress}`);
      logger.log(`connectPort to....${connectPort}`);
      const CONNECT_TV_COMMAND = `${util.TOOLS_SDB_PATH} connect ${deviceIpAddress}:${connectPort}`;
