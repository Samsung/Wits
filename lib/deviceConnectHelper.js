...
module.exports = {
    ...
    getAppInstallPath: deviceName => {
        ...
        let capability = shelljs
            .exec(`${util.TOOLS_SDB_PATH} -s ${deviceName} capability`, { silent: true })
            .split('\n');
...
function getConnectedDeviceList() {
    let devices = shelljs.exec(`${util.TOOLS_SDB_PATH} devices`, { silent: true }).stdout;
    let devicesInfo = [];
    let deviceNameList = [];
