...
module.exports = {
    ...
    connectDevice: deviceIpAddress => {
        ...
        let connectTvCommand = `${util.TOOLS_SDB_PATH} connect ${deviceIpAddress}:${DEFAULT_PORT}`;
        if (deviceIpAddress === EMULATOR_IP || util.isIpPort(deviceIpAddress)) {
            connectTvCommand = `${util.TOOLS_SDB_PATH} connect ${deviceIpAddress}`;
        }

        logger.log(`connect to....${deviceIpAddress}`);
        const connectResult = execSync(connectTvCommand).toString();
