...
module.exports = {
    openSocketServer: async (baseAppPath, deviceInfo, socketPort) => {
        ...
        http.listen(socketPort, optionalInfo.hostIp, () => {
...
function pushFile(baseAppPath, hostAppPath, filesInfo) {
    if (filesInfo.curIdx >= filesInfo.totalFileNum) {
        ...
        const CONTENT_FILE_PUSH_COMMAND = `sdb -s ${deviceName} push "${filePath}" "${hostAppPath}${RemovedBaseAppPath}"`;
