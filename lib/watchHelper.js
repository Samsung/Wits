...
function getWitsIgnore() {
    ...
    try {
        ignore = fs.readFileSync(file,'utf8').replace(regExp.FIND_ALL_CR,'').split('\n');
    }
    catch(e) {
...
function pushFile(baseAppPath,hostAppPath,filesInfo) {
    if(filesInfo.curIdx >= filesInfo.totalFileNum) {
        ...
        let filePath = path.isAbsolute(file) ? file.replace(regExp.BACKSLASH, '/') : getAbsolutePath(file);
...
function watchAppCode(basePath,destPath) {
    watcher = watch(basePath, {recursive: true}, (evt, name) => {
        ...
        let filePath = name.replace(regExp.BACKSLASH, '/').replace(basePath,'');
