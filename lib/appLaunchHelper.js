...
module.exports = {
    installPackage: (deviceInfo, hostAppName) => {
        ...
        const WGT_FILE_PUSH_COMMAND = `${util.TOOLS_SDB_PATH} -s ${deviceName} push "${path.join(PACKAGE_BASE_PATH, WITS_PACKAGE)}" "${appInstallPath}"`;
    ...
    unInstallPackage: (deviceName, hostAppName) => {
        ...
        const result = execSync(APP_UNINSTALL_COMMAND, { silent: true }).toString();
    ...
    launchApp: (deviceName, hostAppId) => {
        ...
        let result = execSync(APP_LAUNCH_COMMAND).toString();
        if (result.includes('failed[')) {
...
function setPortForward(port) {
    const LOCAL_HOST = '127.0.0.1';
    execSync(`${util.TOOLS_SDB_PATH} -s ${deviceName} forward --remove tcp ${port}`);
    execSync(`${util.TOOLS_SDB_PATH} -s ${deviceName} forward tcp:${port} tcp:${port}`);
