...
function setPortForward(deviceName, port) {
    ...
    try {
        ...
        logger.log(
            `Chrome browser can't be launched. Please check if it's installed. If it's still not working, please use the inspector with inputting ${LOCAL_HOST}:${port} into the address bar of the chromium based browser. ${e}`
        );
    }
}
...
function launchChrome(url) {
    chromeLauncher
        ...
        .catch(e => {
            logger.log(chalk.red(`Chrome browser can't be launched. Please check if it's installed. ${e}`));
        });
}
