...
module.exports = {
    installPackage: (deviceInfo, hostAppName) => {
        ...
        const WGT_FILE_PUSH_COMMAND = `${util.TOOLS_SDB_PATH} -s ${deviceName} push  ${PACKAGE_BASE_PATH} ${WITS_PACKAGE} ${appInstallPath}`;
...
function setPortForward(port) {
    const LOCAL_HOST = '127.0.0.1';
    shelljs.exec(`${util.TOOLS_SDB_PATH} -s ${deviceName} forward --remove tcp ${port}`);
    shelljs.exec(`${util.TOOLS_SDB_PATH} -s ${deviceName} forward tcp:${port} tcp:${port}`);
