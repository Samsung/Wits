...
module.exports = {
    ...
    removeFile: filepath => {
        if (fs.existsSync(filepath)) {
            ...
            try {
                ...
                console.log(`Failed to removeFile ${filepath} ${e}`);
                throw e;
            }
        }
    },

    removeFiles: async dirPath => {
        try {
            if (module.exports.isFileExist(dirPath)) {
                const list = await recursiveReadDir(dirPath);
                ...
                for (let i = 0; i < list.length; i++) {
                    ...
                    if (type.isDirectory()) {
                        module.exports.removeFiles(filePath);
                    } else {
                        fs.unlinkSync(filePath);
                    }
                }
            ...
            console.log(`Failed to removeFiles: ${e}`);
        }
    },
    ...
    moveFile: (src, dest) => {
        try {
            ...
            console.log(`Failed to moveFile: ${e}`);
            throw e;
        }
    },
    ...
    copyFiles: async (src, dest) => {
        try {
            ...
            if (module.exports.isFileExist(src)) {
                const list = await recursiveReadDir(src);

                for (let i = 0; i < list.length; i++) {
                    const srcPath = path.join(src, list[i]);
                    const destPath = path.join(dest, list[i]);
                    const type = fs.statSync(srcPath);
                    console.log(`@@@@@@@@@@@ srcPath ${srcPath}`);
                    console.log(`@@@@@@@@@@@ destPath ${destPath}`);

                    console.log(`@@@@@@@@@@@ type ${type}`);
                    if (type.isDirectory()) {
                        module.exports.copyFiles(srcPath, destPath);
                    } else {
            ...
            console.log(`Failed to copyFiles: ${e}`);
        }
    },
    ...
    copyFile: (src, dest) => {
        try {
            ...
            console.log(`Failed to copyFile: ${e}`);
            throw e;
        }
    },
