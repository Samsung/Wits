...
async function getUserAnswer() {
    ...
    if (!(await getConfirmAskData())) {
        ...
        let hostIp = util.isPropertyExist(answer, 'hostIp')
            ? answer.hostIp
            : cInfo.hostIp;
...
async function getUserAskData() {
    ...
    let hostIpQuestion = gethostIpQuestion();
    ...
    if (!util.isPropertyExist(cInfo, 'hostIp')) {
        hostIpQuestion !== null &&
            hostIpQuestion.type &&
            ask.unshift(hostIpQuestion);
    }

    return ask;
}

function gethostIpQuestion() {
    let question = {};
