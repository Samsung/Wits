...
module.exports = {
    ...
    getRefinedData: () => {
        ...
        const baseAppPath = module.exports.getBaseAppPath(
            cInfo.baseAppPath
        );
    ...
    updateLatestUserAnswer: async userAnswer => {
        ...
        let latestProfileInfo = {
            name: userAnswer.profileName ? userAnswer.profileName : pInfo.name,
            path: userAnswer.profilePath ? userAnswer.profilePath : pInfo.path
        };

        if (userAnswer.baseAppPath) {
            latestConnectionInfo['baseAppPath'] =
...
async function getUserAskData() {
    ...
    ask.push(getProfileNameQuestion(pInfo));
...
function getProfileNameQuestion(pInfo) {
    return {
        type: 'input',
        name: 'profileName',
    ...
    if (connectionInfo.baseAppPaths) {
        if (connectionInfo.baseAppPaths.length === 1) {
            ...
            question = {
                ...
                default:
                    baseAppPathIndex >= 0 ? baseAppPathIndex : 0
            };
        }
    }
    return question;
