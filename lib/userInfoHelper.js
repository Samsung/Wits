...
function abortOnConfigSecurityError(e) {
    if (e instanceof pathSafety.ConfigValidationError) {
        logger.error(
            chalk.red(
                `[security] Refusing to use .witsconfig.json: ${e.message}`
            )
        );
        util.exit();
        return true;
...
module.exports = {
  ...
  getLatestWitsconfigInfo: () => {
    ...
    try {
      ...
      pathSafety.validateWitsConfig(wInfo, {
        cwd: util.CURRENT_PROJECT_PATH
      });
      ...
      if (abortOnConfigSecurityError(e)) {
        return result;
      }
  ...
  getOptionalInfo: async () => {
    ...
    if (data !== '' && typeof data === 'string') {
      ...
      try {
        pathSafety.validateWitsConfig(witsConfigData, {
          cwd: util.CURRENT_PROJECT_PATH
        });
      } catch (e) {
        if (abortOnConfigSecurityError(e)) {
