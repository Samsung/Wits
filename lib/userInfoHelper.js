...
module.exports = {
    prepareWitsSetting: async () => {
        let userAnswer = await getUserAnswer();
        let connectionInfo = getWitsconfigData().connectionInfo;

        if (util.isPropertyExist(connectionInfo, 'pcIp')) {
            userAnswer.pcIp = connectionInfo.pcIp;
        }
    ...
    getLatestWitsconfigInfo: () => {
        ...
        try {
            ...
            if (util.isPropertyExist(cInfo, 'pcIp')) {
                connectionInfo.pcIp = cInfo.pcIp;
            }
...
async function getUserAnswer() {
    ...
    if (!(await getConfirmAskData())) {
        ...
        let pcIp = util.isPropertyExist(answer, 'ipv4Addresses')
            ? answer.ipv4Addresses
            : ip.address();
        if (util.isPropertyExist(connectionInfo, 'pcIp')) {
            pcIp = connectionInfo.pcIp;
        }
...
async function getUserAskData() {
    ...
    let ipv4Question = getIpv4AddressQuestion();
    ...
    if (!util.isPropertyExist(connectionInfo, 'pcIp')) {
        ipv4Question !== null && ipv4Question.type && ask.unshift(ipv4Question);
    }

    return ask;
}

function getIpv4AddressQuestion() {
    let question = {};
    let addresses = util.getIpv4Addresses();
...
async function saveLatestWitsconfigInfo(latestInfo) {
    ...
    if (latestInfo.pcIp) {
        latestConnectionInfo.pcIp = latestInfo.pcIp;
    }
