...
module.exports = {
    getUserAnswer: async function() {
        ...
        baseAppPath = path.isAbsolute(answer.baseAppPath) ? answer.baseAppPath.replace(REG_BACKSLASH, '/') : util.getAbsolutePath(answer.baseAppPath);
...
function getUserAskData() {
    ...
    let ask = [{
        ...
        validate: function (input) {
            return util.isIpAddress(input) ? true : 'Invalid format of Ip address which is entered.';
        }
    },{
        type: 'input',
        name: 'port',
        ...
        validate: function (input) {
            return REG_EXP.NUMBER_ONLY.test(input) ? true : 'Invalid type of port which is entered. Must input number type.';
        }
    },{
        type: 'confirm',
        name: 'isDebugMode',
...
function getRecentlyConnectionInfo() {
    try {
        let info = JSON.parse(fs.readFileSync(path.join(__dirname, '../connectionInfo.json'),'utf8'));
        return {
...
function setRecentlyConnectionInfo(recentlyInfo) {
    ...
    try {
        fs.writeFileSync(path.join(__dirname, '../connectionInfo.json'), JSON.stringify(recentlyConnectionInfo,null, 2), 'utf8');
    }
    catch (e) {
