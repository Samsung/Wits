...
module.exports = {
    ...
    getRefinedData: () => {
        ...
        if (util.isIpPort(result.deviceIp)) {
            const device = result.deviceIp.split(':');
            util.DEVICE_INFO.ip = device[0];
            util.DEVICE_INFO.port = device[1];
        } else {
            util.DEVICE_INFO.ip = result.deviceIp;
        }
    ...
    getDeviceInfo: async () => {
        let deviceIp = util.DEVICE_INFO.ip ? util.DEVICE_INFO.ip : null;
...
function getWitsconfigData() {
    try {
        if (
            module.exports.WITS_USER_DATA &&
            module.exports.WITS_USER_DATA !== null
        ) {
...
function getDeviceIpQuestion(cInfo) {
    return {
        ...
        validate: function (input) {
            return util.isIpAddress(input) || util.isIpPort(input)
                ? true
