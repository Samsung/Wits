...
module.exports = {
    getWitsSettingInfo: async () => {
        ...
        let userAnswer = isConfirmed
            ? getRecentlyConnectionInfo()
            : await getUserAnswer();
        let deviceInfo = await deviceConnectHelper.getConnectedDeviceInfo(
            userAnswer.deviceIpAddress
        );
        return {
...
async function getUserAnswer() {
    ...
    let baseAppPath = path.isAbsolute(answer.baseAppPath)
    ...
    let ask = [
        {
            ...
            name: 'isCorrectInfo',
            message:
                '.witsignore is already exist. Do you want to use this data for running Wits?',
            default: true
        }
    ];
    let answer = await inquirer.prompt(ask);
...
function setRecentlyConnectionInfo(recentlyInfo) {
    ...
    try {
        fs.writeFileSync(
            path.join(util.CURRENT_PROJECT_PATH, 'connectionInfo.json'),
