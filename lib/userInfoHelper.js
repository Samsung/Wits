...
async function getUserAnswer() {
    let isConfirmed = await getConfirmAskData();

    let result = {};
    if (isConfirmed) {
        let cInfo = getLatestWitsconfigInfo().connectionInfo;

        result.baseAppPath = getBaseAppPath(cInfo.recentlyBaseAppPath);
        result.hostWidth = cInfo.width;
        result.deviceIpAddress = cInfo.ip;
        result.socketPort = cInfo.port;
...
function getLatestWitsconfigInfo() {
    ...
    try {
        ...
        (connectionInfo.baseAppPath = cInfo.baseAppPaths),
...
async function setLatestWitsconfigInfo(latestInfo) {
    let savingInfo = {};
