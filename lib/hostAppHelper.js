...
module.exports = {
    setHostAppEnv: async (userAnswer, deviceInfo) => {
        ...
        await setBaseFsWrapper(userAnswer, deviceInfo);
    ...
    getHostAppId: baseAppPath => {
        try {
            ...
            console.error(
                chalk.red(`Failed to read base app config.xml. ${e}`)
            );
            util.exit();
        }
    },
...
async function makeHostAppConfigFile(baseAppPath) {
    ...
    try {
        ...
        console.error(chalk.red(`Failed to read user config.xml. ${e}`));
        util.exit();
        return;
    }
    ...
    try {
        ...
        console.error(chalk.red(`Failed to write Wits config.xml. ${e}`));
        util.exit();
        return;
    }
}
...
async function setBaseJSData(userAnswer, deviceInfo) {
    try {
        ...
        console.error(chalk.red(`Failed to set Wits baseJS data to file ${e}`));
        util.exit();
        return;
    }
}
...
function setBaseHtmlData(userAnswer) {
    try {
        ...
        console.error(chalk.red(`Failed to set Wits baseHtml data to file`));
        util.exit();
    }
}
...
async function setBaseFsWrapper(userAnswer, deviceInfo) {
    try {
        const file = path.join(
            util.WITS_BASE_PATH,
            'wrapper',
