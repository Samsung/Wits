    ...
    buildPackage: async() => {
        ...
        await new Promise(async (resolve, reject) => {
            ...
            output.on('close', async () => {
                ...
                util.removeFile(path.resolve(path.join(outputTempPath, AUTOR_SIGNATURE)));
                util.removeFile(path.resolve(path.join(outputTempPath, PUBLIC_SIGNATURE)));
                util.removeFile(path.resolve(path.join(outputTempPath, PUBLIC_SIGNATURE2)));
                util.removeFile(path.resolve(path.join(outputTempPath, MAINFEST_TMP)));
                
                util.removeFile(path.resolve(path.join(workspacePath, AUTOR_SIGNATURE)));
                util.removeFile(path.resolve(path.join(workspacePath, PUBLIC_SIGNATURE)));
                util.removeFile(path.resolve(path.join(workspacePath, PUBLIC_SIGNATURE2)));
                util.removeFile(path.resolve(path.join(workspacePath, MAINFEST_TMP)));
                
                fse.removeSync(path.resolve(path.join(workspacePath), RESOURCES));
...
function preparePackage() {
    ...
    try {
        ...
        console.log(`Failed to signPackage ${ex}`);
    }
}
