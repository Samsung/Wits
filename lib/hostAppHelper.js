...
module.exports = {
    ...
    getHostAppId: () => {
        try {
            ...
            let id = data.match(regExp.APPLICATION_ID)[0].replace(regExp.APPLICATION_ID_ATTRIBUTE,'');
            return id;
        }
        catch(e) {
...
function setBaseJSData(userAnswer,deviceInfo) {
    try {
        ...
        let contentFullSrc = util.isRemoteUrl(contentSrc)? contentSrc : (hostAppPath + '/' + contentSrc.replace(regExp.FISRT_BACKSLASH,''));
        let convertData = {
...
function getContentSrc(baseAppPath) {
    ...
    try {
        ...
        let data = fs.readFileSync(file,'utf8');
        data = util.clearComment(data);
        contentSrc = data.match(regExp.CONTENT_SRC)[0].replace(regExp.CONTENT_SRC_ATTRIBUTE,'');
    }
    catch(e) {
