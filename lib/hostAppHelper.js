...
module.exports = {
    ...
    buildPackage: profileInfo => {
        ...
        try {
            let www = path.join(util.WITS_BASE_PATH, "../", "tizen");
            let dest = path.join(util.WITS_BASE_PATH, "../", "tizen", "build");
...
async function makeHostAppConfigFile(baseAppPath) {
    ...
    try {
        fs.writeFileSync(
            path.join(util.WITS_BASE_PATH, "../", "tizen", CONFIG_FILE),
            witsConfigData,
            "utf8"
        );
    } catch (e) {
...
function setBaseJSData(userAnswer, deviceInfo) {
    try {
        ...
        fs.writeFileSync(
            path.join(util.WITS_BASE_PATH, "../", "tizen", "js", "main.js"),
            str,
            "utf8"
        );
    } catch (e) {
...
function setBaseHtmlData(userAnswer) {
    try {
        let file = path.join(util.WITS_BASE_PATH, "../", "tizen", "base.html");
        let data = fs.readFileSync(file, "utf8");

        let str = data.replace(regExp.HOST_WIDTH, userAnswer.hostWidth);

        fs.writeFileSync(
            path.join(util.WITS_BASE_PATH, "../", "tizen", "index.html"),
            str,
            "utf8"
        );
    } catch (e) {
