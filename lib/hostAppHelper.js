    ...
    buildPackage: async() => {
        ...
        await new Promise(async (resolve, reject) => {
            ...
            output.on('close', () => {
                ...
                var authorSignature = path.resolve(
                    path.join(outputTempPath, AUTOR_SIGNATURE)
                );
                var publicSignature = path.resolve(
                    path.join(outputTempPath, PUBLIC_SIGNATURE)
                );
                var publicSignature2 = path.resolve(
                    path.join(outputTempPath, PUBLIC_SIGNATURE2)
                );
                var tmpFile = path.resolve(path.join(outputTempPath, MAINFEST_TMP));
            ...
            archive.pipe(output);
            // const files = await getFilesToPackage(workspacePath);
            ...
            archive.bulk([{
                // {
                //     src: [path.join(util.WITS_BASE_PATH, '../', 'container', '*')],
                //     dest: path.join(outputTempPath),
...
async function getFilesToPackage(workspacePath) {
    ...
    console.log('@@@@@@@@@@@@', await recursiveReadDir(workspacePath, ignoreFiles));
}
